...
exports.setup = (mstream) => {
  mstream.post("/api/v1/auth/login", async (req, res) => {
    try {
      ...
      if (!config.program.users[req.body.username]) {
        throw new Error("user not found");
      }

      await auth.authenticateUser(
        config.program.users[req.body.username].password,
        config.program.users[req.body.username].salt,
        req.body.password
      );
      ...
      winston.warn(
        `Failed login attempt from ${req.ip}. Username: ${req.body.username}`,
        { stack: err }
      );
      setTimeout(() => {
  ...
  mstream.use((req, res, next) => {
    ...
    try {
      ...
      throw new WebError("Token Expired", 401);
    }
    ...
    if (decoded.invite && decoded.invite === true) {
      ...
      if (req.path === "/federation/invite/exchange") {
        return next();
      }
      throw new WebError("Authentication Error", 401);
    }
    ...
    if (decoded.shareToken && decoded.shareToken === true) {
      ...
      if (
        ...
        playlistItem.playlist.indexOf(decodeURIComponent(req.path).slice(7)) ===
          -1
      ) {
        throw new WebError("Authentication Error", 401);
      }
